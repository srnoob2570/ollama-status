...
Cloudflare Worker and React dashboard that monitors the Ollama Cloud free and paid accounts. It discovers one global catalog, probes each model with both accounts without retaining output, groups models by free or paid entitlement, publishes public status, and protects administration through Cloudflare Access.
...
The dashboard is public at `/`; APIs are under `/api/v1`. `/admin/*` and `/api/admin/*` require a valid Cloudflare Access JWT, in addition to the Access policy at the edge.
...
Use `wrangler secret put --env <staging|production>` for `OLLAMA_API_KEY_FREE`, `OLLAMA_API_KEY_PAID`, alert webhook URLs, Access values, and confirmation secrets. Do not put secrets in `wrangler.jsonc`, D1, or browser code.
