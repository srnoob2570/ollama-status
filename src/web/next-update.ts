...
export function nextUpdateLabel(nextCheckAt: string | null, updating: boolean, currentTime = Date.now()): string {
  ...
  const remainingSeconds = Math.max(0, Math.ceil((Math.max(scheduledAt, nextCronBoundary) - currentTime) / 1_000))
