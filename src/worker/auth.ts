...
export async function requireAccess(request: Request, env: Env): Promise<{ email: string }> {
    ...
    try {
        header = JSON.parse(new TextDecoder().decode(decode(encodedHeader)));
        payload = JSON.parse(new TextDecoder().decode(decode(encodedPayload)));
    } catch {
        throw new Response('Forbidden', { status: 403 });
    }
    ...
    if (
        ...
        throw new Response('Forbidden', { status: 403 });
    const certs = await fetch(`https://${env.ACCESS_TEAM_DOMAIN}/cdn-cgi/access/certs`).then((r) =>
        r.ok
            ? (r.json() as Promise<{ keys: Array<JsonWebKey & { kid?: string }> }>)
            : Promise.reject(new Error('certs_unavailable')),
    );
    ...
    if (
        !(await crypto.subtle.verify(
            'RSASSA-PKCS1-v1_5',
            key,
            decode(encodedSignature) as unknown as BufferSource,
