...
CREATE TABLE IF NOT EXISTS model_current_status (
...
CREATE INDEX IF NOT EXISTS idx_incidents_model_status ON incidents(model_id, status, started_at DESC);
CREATE TABLE IF NOT EXISTS hourly_model_rollups (
...
CREATE TABLE IF NOT EXISTS scheduler_locks (name TEXT PRIMARY KEY, lease_until TEXT NOT NULL, owner TEXT NOT NULL, updated_at TEXT NOT NULL);
CREATE TABLE IF NOT EXISTS monitor_runs (id TEXT PRIMARY KEY, started_at TEXT NOT NULL, finished_at TEXT, outcome TEXT, detail TEXT);
CREATE TABLE IF NOT EXISTS notification_deliveries (id TEXT PRIMARY KEY, incident_id TEXT NOT NULL REFERENCES incidents(id), destination TEXT NOT NULL, event_type TEXT NOT NULL, delivered_at TEXT, attempts INTEGER NOT NULL DEFAULT 0, last_error TEXT);
CREATE TABLE IF NOT EXISTS audit_log (id TEXT PRIMARY KEY, occurred_at TEXT NOT NULL, actor TEXT NOT NULL, action TEXT NOT NULL, target_type TEXT NOT NULL, target_id TEXT, detail TEXT);
CREATE TABLE IF NOT EXISTS region_confirmations (id TEXT PRIMARY KEY, incident_id TEXT NOT NULL REFERENCES incidents(id), nonce TEXT NOT NULL UNIQUE, status TEXT NOT NULL DEFAULT 'PENDING', result_classification TEXT, received_at TEXT, expires_at TEXT NOT NULL);
